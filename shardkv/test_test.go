...
func TestConcurrent1_5Z(t *testing.T) {
...
func TestUnreliable1_5B(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		ka[i] = strconv.Itoa(i) // ensure multiple shards
		va[i] = randstring(5)
		fmt.Printf("put 1 %v %v\n\n", ka[i], va[i])
		ck.Put(ka[i], va[i])
		fmt.Printf("put 2 %v %v\n\n", ka[i], va[i])
	}
	fmt.Printf("pass 1\n\n")
	cfg.join(1)
	cfg.join(2)
	...
	for ii := 0; ii < n*2; ii++ {
		...
		x := randstring(5)
		fmt.Printf("append 1 %v %v\n\n", ka[i], x)
		ck.Append(ka[i], x)
		fmt.Printf("append 2 %v %v\n\n", ka[i], x)
		va[i] += x
	}
	fmt.Printf("pass 2\n\n")
	cfg.join(0)
