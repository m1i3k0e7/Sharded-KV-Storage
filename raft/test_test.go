...
func TestManyElections3A(t *testing.T) {
	...
	for ii := 1; ii < iters; ii++ {
		...
		cfg.disconnect(i2)
		cfg.disconnect(i3)
		// DPrintf("disconnect: %d, %d, %d", i1, i2, i3)
...
func TestFollowerFailure3B(t *testing.T) {
	...
	DPrintf("disconnect %d %d", (leader2 + 1) % servers, (leader2 + 2) % servers)
	// submit a command.
...
func snapcommon(t *testing.T, name string, disconnect bool, reliable bool, crash bool) {
	...
	for i := 0; i < iters; i++ {
		...
		if disconnect {
			...
			// if victim == leader1 {
			// 	DPrintf("disconnect leader %d", victim)
			// } else {
			// 	DPrintf("disconnect %d", victim)
			// }
			cfg.one(rand.Int() % 1000, servers-1, true)
		}
		if crash {
			cfg.crash1(victim)
		...
		for i := 0; i < nn; i++ {
			cfg.rafts[sender].Start(rand.Int() % 1000)
		}
		// DPrintf("pass 2")
		// PRINT = true
		...
		if disconnect {
			...
			cfg.connect(victim)
			// DPrintf("connect disconnect %d", victim)
			cfg.one(rand.Int() % 1000, servers, true)
			leader1 = cfg.checkOneLeader()
		}
		if crash {
			...
			cfg.connect(victim)
			// DPrintf("connect crash %d", victim)
			cfg.one(rand.Int() % 1000, servers, true)
			leader1 = cfg.checkOneLeader()
		}
	}
