...
func checkClntAppends(t *testing.T, clnt int, v string, count int) {
	...
	for j := 0; j < count; j++ {
		...
		if off1 != off {
			t.Fatalf("server %d: duplicate element %v in Append result", clnt, wanted)
		}
		if off <= lastoff {
			t.Fatalf("server %d: wrong order for element %v in Append result", clnt, wanted)
		}
		lastoff = off
	}
}
...
func GenericTest(t *testing.T, part string, nclients int, nservers int, unreliable bool, crash bool, partitions bool, maxraftstate int, randomkeys bool) {
	...
	for i := 0; i < 3; i++ {
		...
		go spawn_clients_and_wait(t, cfg, nclients, func(cli int, myck *Clerk, t *testing.T) {
			...
			for atomic.LoadInt32(&done_clients) == 0 {
				...
				if (rand.Int() % 1000) < 500 {
					...
					// DPrintf("append start %v", key)
					Append(cfg, myck, key, nv, opLog, cli)
					// DPrintf("append end %v", key)
					if !randomkeys {
					...
					// DPrintf("get start %v", key)
					v := Get(cfg, myck, key, opLog, cli)
					// DPrintf("get end %v", key)
		...
		for i := 0; i < nclients; i++ {
			...
			// DPrintf("%d get2 start", i)
			v := Get(cfg, ck, key, opLog, 0)
			// DPrintf("%d get2 finish", i)
			if !randomkeys {
...
func TestSnapshotRecoverManyClients4X(t *testing.T) {
